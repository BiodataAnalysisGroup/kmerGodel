...
dir.create('plots')
dir.create('plots/Jeffreys')
output_folder <- 'plots/Jeffreys/'
...
for (enc in encodings){
  ...
  jeffreys_vector_sars <- future_lapply(kvals, function(k) {
    ...
    tmp <- jeffreys_divergenceOfDistributions(godels_1, godels_2, k, folder)
    tmp
    
  })
  d = cbind(d, jeffreys_vector_sars)
}

rownames(d) <- kvals
colnames(d) <- legend_list

output_filename <- paste(output_folder, 'jeffreys_two_distributions_divergence_',kmin,'_',kmax,'.csv', sep = '')
