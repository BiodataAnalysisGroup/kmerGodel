...
# Source
source("two_dists_helpin_functions.R")
...
dir.create('plots')
dir.create('plots/hellinger')
output_folder <- 'plots/hellinger/'
...
for (enc in encodings){
  ...
  hellinger_vector_sars <- future_lapply(kvals, function(k) {
    ...
    tmp <- hellinger_divergenceOfDistributions(godels_1, godels_2, k, folder)
    tmp
    
  })
  d = cbind(d, hellinger_vector_sars)
}

rownames(d) <- kvals
colnames(d) <- legend_list

output_filename <- paste(output_folder, 'hellinger_two_distributions_divergence_',kmin,'_',kmax,'.csv', sep = '')
